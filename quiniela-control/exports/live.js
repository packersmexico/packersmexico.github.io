...
let current;
async function load(){const r=await fetch(DATA+'?t='+Date.now(),{cache:'no-store'});current=await r.json();document.getElementById('asset-week').textContent='QUINIELA · '+current.week+' · '+current.season;document.getElementById('asset-status').textContent='PUBLICATION READY · '+current.capture.complete+'/'+current.capture.total+' · '+current.results.final+'/'+current.results.total+' FINAL';renderDetail(current);renderWeekly(current);renderSeason(current)}
document.querySelectorAll('[data-download]').forEach(btn=>btn.addEventListener('click',()=>{const [w,h]=btn.dataset.size.split('x').map(Number),id=btn.dataset.download,name=id==='detail-svg'?'PMX_'+current.week.replace(' ','_')+'_TODOS_LOS_PICKS.png':id==='weekly-svg'?'PMX_'+current.week.replace(' ','_')+'_RESULTADOS_RANKING.png':'PMX_RANKING_GENERAL_'+current.week.replace(' ','_')+'.png';svgToPng(document.getElementById(id),name,w,h)}));
